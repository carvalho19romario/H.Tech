  ...
  const updateContent = useCallback(async (newContent: SiteContent) => {
    ...
    try {
      ...
      if (existing) {
        await supabase
          .from('site_content')
          .update({ data: newContent as unknown as Record<string, unknown>, updated_at: new Date().toISOString() })
          .eq('id', 'main');
      } else {
        await supabase
          .from('site_content')
  ...
  const resetContent = useCallback(async () => {
    ...
    try {
      ...
      if (existing) {
        ...
        await supabase
          .from('site_content')
          .insert({ id: 'main', data: defaultContent as unknown as Record<string, unknown> });
      }
    } catch (e) {
